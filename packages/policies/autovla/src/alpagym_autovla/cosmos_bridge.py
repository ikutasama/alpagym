...
class Qwen2_5_VLWeightMapper(HFModelWeightMapper):
    ...
    def rollout_split_local_key_n_param_to_hf_key_n_param(
        ...
        # Visual encoder qkv weights must be split into q/k/v to match the
        # policy side's policy_decompose_param_1_to_n_for_sync which splits
        # visual.blocks.*.attn.qkv → q/k/v.  The parent
        # HFModelWeightMapper.rollout_split already handles this split
        # correctly (elif "qkv" in compatible_key and "visual" in
        # compatible_key), so we must NOT early-return here.
