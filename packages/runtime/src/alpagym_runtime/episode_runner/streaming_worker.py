...
class StreamingRolloutWorker:
    ...
    def _rollout_worker_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                logger.exception(
                    "Worker thread survived uncaught exception in _run_rollout"
                )
    ...
    def _run_rollout(self, rollout_job: _RolloutJob) -> None:
        ...
        try:
            ...
            ade_vals = [
                po.replay_data.payload["ade"]
                for po in record.outputs
                if po.replay_data is not None
                and "ade" in po.replay_data.payload
            ]
            fde_vals = [
                po.replay_data.payload["fde"]
                for po in record.outputs
                if po.replay_data is not None
                and "fde" in po.replay_data.payload
            ]
            if ade_vals:
                import statistics as _stats
                logger.info(
                    "Episode ADE/FDE session=%s scene=%s "
                    "ade_mean=%.3f ade_min=%.3f ade_max=%.3f "
                    "fde_mean=%.3f fde_min=%.3f fde_max=%.3f (n=%d/%d)",
                    rollout_job.session_uuid,
                    rollout_job.scene_id,
                    _stats.mean(ade_vals),
                    min(ade_vals),
                    max(ade_vals),
                    _stats.mean(fde_vals),
                    min(fde_vals),
                    max(fde_vals),
                    len(ade_vals),
                    len(record.outputs),
            ...
            try:
                ...
                logger.warning("torch.cuda.empty_cache() failed; ignoring", exc_info=True)
    ...
    def _on_rollout_succeeded(self, rollout_job: _RolloutJob, episode: EpisodeOutput) -> None:
        ...
        # Track a proven-healthy scene so permanently-failed payloads can be
        # retried on a substitute scene instead of leaving a hole in the batch.
        if getattr(rollout_job, "attempts", 0) == 0 and not getattr(
            rollout_job, "is_fallback", False
        ):
            self._last_good_scene_id = rollout_job.scene_id
    ...
    def _on_rollout_failed(self, rollout_job: _RolloutJob, exc: BaseException) -> None:
        ...
        if should_drop:
            # Poison-scene guard: never leave a hole in the batch. Re-dispatch
            # the payload's slot on a proven-healthy substitute scene so the
            # trainer still receives the expected sample count. Without this,
            # a dropped payload permanently starves rollouts_enough_for_one_step
            # and the whole run deadlocks (observed 2026-09-04/07).
            fallback_scene = getattr(self, "_last_good_scene_id", None)
            if fallback_scene is None:
                # Cold-start seed: no successful episode yet in this worker's
                # lifetime. Pull a substitute candidate from the scene pool so
                # the very first poisoned payload still gets a replacement
                # instead of freezing the batch (observed 2026-09-07).
                try:
                    import yaml as _yaml
                    with open(
                        "/data/mnt_m62/10_personal/z59900495/workspace/latest/alpasim_scene_ids.yaml"
                    ) as _f:
                        _ids = (_yaml.safe_load(_f) or {}).get("scene_ids") or []
                    if _ids:
                        fallback_scene = _ids[1] if len(_ids) > 1 else _ids[0]
                        self._last_good_scene_id = fallback_scene
                        logger.info(
                            "Seeded fallback scene from pool: %s", fallback_scene
                        )
                except Exception as _e:
                    logger.warning("Fallback scene seed failed: %s", _e)
                    fallback_scene = None
            if fallback_scene is not None and not getattr(
                rollout_job, "is_fallback", False
            ):
                payload_state.permanently_failed = False
                payload_state.future_resolved = False
                payload_state.retries_left = self._max_scene_retries
                logger.warning(
                    "Substituting poisoned scene=%s with proven-healthy scene=%s "
                    "(prompt_idx=%s)",
                    rollout_job.scene_id,
                    fallback_scene,
